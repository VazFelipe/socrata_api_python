...
def extract_list_of_dates():
    ...
    list_of_dates = [next_day[i] for i in range(delta_date.days+1)]
    return list_of_dates
            
def extract_data():
    list_of_dates = extract_list_of_dates()
    client = Socrata(domain="data.sfgov.org",
                 app_token="",
                 username="",
                 password="")
    query = f'select count(*) where incident_datetime < "{[list_of_dates[i] for i in range(list_of_dates[i])]}]"'
